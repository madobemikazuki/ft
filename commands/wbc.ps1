...
function fn_Registration_Format {
  ...
  [PSCustomObject[]]$private:applicant_list = foreach ($_ in $extracted_targets_info) {
    [PSCustomObject] @{
      ...
      $script:wbc_application_field[2] = $_.'登録WBC_申請会社'
...
$private:config_object = fn_Read $config_path

#ここで必要な情報を集約整形して格納する。
switch ($Task) {
  r { $applicants = fn_Registration_Format $_central_nums $config_object.$Task }
  c { $applicants = fn_Cancellation_Format $_central_nums $config_object.$Task }
}


#$private:application_date
#$applicants | Format-Table
# $applicants.length が1より大きく5より小さければ一回実行
