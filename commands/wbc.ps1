...
function script:fn_Apply_Date {
  ...
  . .\commands\ft_core\future_date.ps1
...
function script:fn_regist_company_names {
  ...
  . .\commands\ft_core\your_company_names.ps1
...
function fn_Generate_WBC_Config_Object {
  ...
  $private:config = . .\commands\ft_core\io\read_json.ps1 ".\commands\config\wbc.json"
...
function fn_Posting_Format {
  ...
  $formated_obj = foreach ($applicant in $_applicants) {
    ...
    if ($applicant.psobject.Properties.value.count -eq $_header.length) {
      ...
      . .\commands\ft_core\excel\map_address.ps1 $_header $position $applicant 
    }
  }
  return $formated_obj
}
...
function fn_Read_AppComs {
  $config = . .\commands\ft_core\io\read_json.ps1 "${HOME}\Downloads\TEMP\coms.json"
...
function fn_Registration_Format {
  ...
  $coms = fn_Read_AppComs

  [PSCustomObject[]]$applicants = foreach ($_ in $extracted_targets_info) {
    $com_name = $coms.($_.'所属企業番号' -replace "^0", "T")# 00294 => T0294
    $shorten_1 = . .\commands\ft_core\shorten_kabu.ps1 $com_name
    $shorten_2 = . .\commands\ft_core\shorten_kabu.ps1 $_.'雇用企業名称（漢字）'
    [PSCustomObject] @{
      $script:wbc_application_field[0] = $_date
      $script:wbc_application_field[1] = $_.'個人番号'
      $script:wbc_application_field[2] = . .\commands\ft_core\your_company_name.ps1 $shorten_1 $shorten_2
      $script:wbc_application_field[3] = . .\commands\ft_core\combined_name.ps1 $_.'漢字氏名（姓）' $_.'漢字氏名（名）'
    }
  }


 
  <# ついでに 主管理情報の集積 accumulate_gzen_info
  正確な情報を収集する
  作業主管グループ
  作業件名コード(WID)
  作業件名（目的等）
$path = .\${HOME}\Downloads\from_T\登録\gzen_collenction\Group_WID.json
作業主管グループ名でソートしてSetオブジェクトとして一意化する。
#>
...
function fn_Cancellation_Format {
  ...
  $registed_list = . .\commands\ft_core\io\read_registed_people_fromT.ps1
  $targets = foreach ($target in $_applicants) {
  ...
  [PSCustomObject[]]$private:applicants = foreach ($_ in $extracted_targets_info) {
    ...
    $shorten_1 = . .\commands\ft_core\shorten_kabu.ps1 $_.($extract_list[4])
    $shorten_2 = . .\commands\ft_core\shorten_kabu.ps1 $_.($extract_list[5])
    ...
    [PSCustomObject] @{
      ...
      $script:wbc_application_field[2] = . .\commands\ft_core\your_company_name.ps1 $shorten_1 $shorten_2
      $script:wbc_application_field[3] = . .\commands\ft_core\combined_name.ps1 $_.'氏名（姓）' $_.'氏名（名）'
    }
  }
...
#ここで必要な情報を集約整形して格納する。
switch ($Task) {
  r { $private:applicants = fn_Registration_Format ([ref]$application_date) $_central_nums }
  c { $private:applicants = fn_Cancellation_Format ([ref]$application_date) ([ref]$_central_nums) }
}
...
$wbc_header = $config_object.printing.address_table[0].psobject.Properties.Name

$object_name_key = '氏名'
$name_list = fn_PSObjList_Filter ([ref]$applicants) @($object_name_key)
$list = foreach ($_ in $name_list) { $_.$object_name_key }
$one_line_names = . .\commands\ft_core\bind_names.ps1 $list
$export_path = fn_Generate_WBC_Output_Path ([ref]$private:config_object) $one_line_names


[PSCustomObject[]]$posting_object = fn_Posting_Format ([ref]$applicants) ([ref]$wbc_header) ([ref]$config_object)
