...
function fn_Open_Excel_Book {
  Param(
    [Parameter(Mandatory = $True, position = 0)]
    [String][ref]$_export_path
  )
  $private:excel = New-Object -ComObject Excel.Application
  $excel.Visible = $True
  $excel.Workbooks.Open($_export_path, 0, $true)
}
...
try {
  ...
  # TODO: 専用のクラスに集約すれば中身の心配は不要だなぁ。
  # 引数 $_path_dict は template と export というプロパティを保持するハッシュです。
  $template_path = $_path_dict.template
  $export_path = $_path_dict.export
  ...
  $script:book = $excel.Workbooks.Open($template_path, 0, $true)
  ...
  if ($print_config.printable) {
    ...
    # 空ファイルを作成し、そこに出力
    New-Item $export_path -type file -Force
    Write-Output "🌸🌸🌸  出力先 :" $export_path  
    $book.SaveAs($export_path)
    $book.Close()
  }
  # TODO: 動作確認できたら以下の else節 はコメントアウトしろ。
  # プリントするまえにExcelファイルの表示が期待通りであるか確認すること。
  else {
    # 空ファイルを作成し、そこに出力
    New-Item $export_path -type file -Force
    $book.SaveAs($export_path)
    
    fn_Open_Excel_Book ([ref]$export_path)
    Write-Output "🌸🌸🌸  出力先 :excel open" $export_path    
    $book.Close()
  }
}
catch [exception] {
