...
function script:fn_WBC_Company_Names {
  ...
  # この処理は bind_gr.ps1 とは若干違う。
  # 理由: 登録者管理リスト_coh.csv のフィールドと事前申請のフィールドが異なるため。
  if ($_applicant.'電力申請会社名称'-eq $_applicant.'雇用名称') { 
    return $_applicant.'電力申請会社名称' 
  }
  if($_applicant.'管理会社名称'.Contains("派遣")){
    return ($_applicant.'管理会社名称' + "／" + $_.'雇用名称')
  }
  return ($_applicant.'電力申請会社名称' + "／" + $_.'雇用名称')
}
...
$script:new_keys = @("中央登録番号","解除予約日", "解除予約時間","担当主管課班名")

$script:extracted_regists = foreach($_reserved in $reserved_obj_list){
  $filterd_regists = fn_Array_Filter $regists_obj_list "中登番号" $_reserved."中央登録番号"
  # 必要最低限度の予約情報だけ抽出
  $addition_reserved_obj =  $_reserved | Select-Object -Property $new_keys
  $extracted = foreach($_regist in $filterd_regists){
...
foreach($_ in $extracted_regists){
  ...
  $company_name = fn_WBC_Company_Names $_
