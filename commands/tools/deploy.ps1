Param(
  ...
  [ValidatePattern("wbc|gZEN|coms|ed|wid|coh|rsv|ft_init|adding|ExcelToCSV|one_array|cnc")]$_command_name
)

Set-StrictMode -Version 3.0
$ErrorActionPreference = "Stop"

$source_folder = "${HOME}\apps\ft"
$config_path = "$source_folder\commands\config\deploy_files.json"
$script_files = @(Get-Content -Path $config_path | ConvertFrom-Json)

$_command_name
# $script_files.$_command_name

$destination_folder = "${HOME}\Downloads\deployed\$_command_name"
