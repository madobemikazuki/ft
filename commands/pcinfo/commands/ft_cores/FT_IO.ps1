...
class FT_IO {
  ...
  static [bool] Validate([String]$_path, [String]$_pattern) {
    $private:result = if ( (Test-Path $_path) -and [FT_IO]::Match_Extension($_path, $_pattern)) {
      $True
    }
    else { 
  ...
  static [void] Write_JSON_Array([String]$_path, [PSCustomObject[]]$_Object_List) {
    ...
    $private:utf8_with_BOM = New-Object System.Text.UTF8Encoding $True
  ...
  static [PSCustomObject[]] Bind_As_CSV([Object[]]$_header, [Object[]]$_values, [String]$_delimiter) {
    [PSCustomObject[]]$private:obj_arr = $_values | ConvertFrom-Csv -Header $_header.Split($_delimiter)
    return $obj_arr
  }
  ...
  static [String[]] Read_ToArray([String]$_txt_path, [String]$_encode) {
    $private:array = if ([FT_IO]::Validate($_txt_path, "\.txt$")) {
  ...
  static [String]Find_Latest_File([String]$_Folder, [String]$_TargetName) {
    $private:file_list = Get-ChildItem -Path $_Folder -File -Filter $_TargetName
    if ($null -eq $file_list) { throw "$_TargetName に該当するファイルが存在しません。" }
    $private:latest_file = ($file_list | Sort-Object LatestWriteTime -Descending)[0].FullName
    return $latest_file
  }
  ...
  static[void] Write_Log([String]$_path, [hashtable]$_log) {
    ...
    if (Test-Path $_path) {
      ...
      $private:ordered_key = $log.keys | Sort-Object -Descending
      $private:new_hash = [ordered]@{}
  ...
  static [String]Find_One([String]$_Folder, [String]$_TargetName) {
    $private:file_path = Get-ChildItem -Path $_Folder -File -Filter $_TargetName
    return $file_path.FullName
  }
}
