...
[PSCustomObject]$private:config = ([FT_IO]::Read_JSON_Object($config_path)).$command_name

[FT_Message]::execution($config.command_name)

[PSCustomObject]$target_file_name = $config.common.source_file_name
$private:source_folder = (${Home} + $config.common.source_folder)
$private:source_file = [FT_IO]::Find_Latest_File($source_folder, $target_file_name)

# Excel のSheet を指定行から値を読むための設定値を FT_StartingRow.json から取得する。
[PSCustomObject]$script:page_config = [FT_IO]::Read_JSON_Object($config.common.starting_row_config_path)
...
try {
  ...
  foreach ($_task in [PScustomObject[]]$config.tasks) {
    ...
    $starting_row = $page_config.($_task.starting_row)
    $end_of_rows = $Sheet.UsedRange.Rows.Count
    Write-Host ("読込み開始行: " + $end_of_rows)
    ...
    #$LookUp_task_dict[$_task.task_initial] = [FT_Array]::Map($sorted_reserved_list, $_task.selection)
    $final_obj = [FT_Array]::Map($sorted_reserved_list, $_task.selection)
    $json_path = (${HOME} + $config.common.export_folder + $_task.export_file)
    [FT_IO]::Write_JSON_Array($json_path, $final_obj)
