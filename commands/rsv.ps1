...
function script:fn_TDTW {
  ...
  #二つの日付の差分から文字列に変換した日付を返す
  # 返す日付のフォーマットはデフォルト値以外も指定できる。
  $private:days_span = (New-TimeSpan $_today $_other_day).Days

  if ($days_span -gt 0) { return $_other_day.ToString($_format) }
  if ($days_span -eq 0) { return $_today.ToString($_format) }
  if ($days_span -lt 0) { return $_other_day.AddYears(+1).ToString($_format) }
}
...
try {
  ...
  foreach ($_task in [PScustomObject[]]$config.tasks) {
    ...
    $reserved_search_key = $field_culumns -match $config.common.flag_search_key
    ...
    [PScustomObject[]]$reserved_list = foreach ($_row in $select_rows_range) {
      ...
      foreach ($_column_number in $column_num_list) {
        ...
        $_value = switch -Regex ($_key) {
          ...
          <#
          $config.common.flag_search_key {
            $_regist_num = $Sheet.Cells.Item($_row, $_colomn_number).Text
            if ([String]::IsNullOrEmpty($_date)) { 
              Start-Process -FilePath notepad.exe -ArgumentList $_path
              break
            }
          }
            #>
          ...
          $config.common.flag_reserved_date { 
            ...
            if ([String]::IsNullOrEmpty($_value_date)) { break }
      ...
      #foreach ($_valid in $is_Null_or_Empty) { if ($_valid) { continue } }
