...
function script:fn_Find_Latest_File {
  ...
  # 指定したフォルダ内から、指定した名前を含むファイル群から最新の更新日のファイルを返す
  $file_list = (Get-ChildItem -Path $_Folder -File -Include $_TargetName).fullname
  $latest_file = ($file_list | Sort-Object LatestWriteTime -Descending)[0] 
  return $latest_file
}

function private:fn_Write_JSON{
  Param(
    [Parameter(Mandatory = $True, Position = 0)]
    [String]$_path,
    [Parameter(Mandatory = $True, Position = 1)]
...
$wid_file = fn_Find_Latest_File $source_folder $name
#$wid_file
