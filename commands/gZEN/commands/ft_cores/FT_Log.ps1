class FT_Log {
  ...
  static [bool] Validate ([String]$_path, [String]$_pattern) {
    $result = ((Test-Path $_path) -and [FT_Log]::Match_Extension($_path, $_pattern)) 
    return $result
  }
  ...
  static[void] Write([String]$_path, [hashtable]$_log) {
    ...
    $path = $_path
    if ([FT_Log]::Validate($path, [FT_Log]::extension_pattern)) {
      $log = $_log
      [PSCustomObject]$exists = [FT_Log]::Read_JSON($path)
      ...
      [FT_Log]::Write_JSON($path, [PSCustomObject]$new_hash)
    }
    else {
      if ([FT_Log]::Match_Extension($path, [FT_Log]::extension_pattern)) {
        [FT_Log]::Write_JSON($path, [PSCustomObject]$_log)
      }
    }
  }
}
