...
function fn_init {
  . .\ft_core\io\read_json.ps1 .\config\gZEN.json
}
...
function fn_shape_values {
  ...
  $new_obj = $arg | Select-Object *, @{
    Name       = 'カタカナ氏名';
    Expression = { . .\ft_core\combined_name.ps1 $_.'カナ氏名（姓）' $_.'カナ氏名（名）' }
  }
  $new_obj = $new_obj | Select-Object *, @{
    Name       = '漢字氏名';
    Expression = { . .\ft_core\combined_name.ps1 $_.'漢字氏名（姓）' $_.'漢字氏名（名）' }
  }

  foreach ($_ in $new_obj) {
    # 郵便番号変換処理は不要かも
    $_.'現住所（住民票）郵便番号' = . .\utils\post_code.ps1 $_.'現住所（住民票）郵便番号'
    $_.'現住所（現在住んでいる）郵便番号' = . .\utils\post_code.ps1 $_.'現住所（現在住んでいる）郵便番号'
    $_.'現住所（住民票）住所' = . .\utils\hanzen.ps1 wide $_.'現住所（住民票）住所'
...
#>
  ...
  $header = . .\ft_core\io\read_to_array.ps1 $config.orign_header
 
  # *事前申請*.txt はSHIFT-JISでエンコードされている。
  $temps = $config.temp_folder
  #$config.gZEN_targets
  $values_filenames = . .\ft_core\io\find_files.ps1 "${HOME}$temps\*" $config.gZEN_targets
  #$values_filenames

  $values = foreach ($_ in $values_filenames) {
    . .\ft_core\io\read_to_array.ps1 $_
  }
  #$header.length
  #$values.length

  #テキストファイルを読み込み、ヘッダーをつけてCSVファイルを生成。
  [PSCustomObject[]]$csv_obj = . .\ft_core\io\bind_as_csv.ps1 $header $values

  # 必要な項目の情報を抽出する
  $sorted_list = . .\ft_core\io\read_to_array.ps1 $config.sorted_header
  ...
  #CSVファイルを出力
  . .\ft_core\io\export_csv.ps1 $new_csv_obj $destination
