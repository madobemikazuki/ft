...
function script:fn_Insert_Reserved_Date {
  Param(
    [Parameter(Mandatory = $True, Position = 0)]
  ...
  $target_prop = '中央登録番号'
  $applicants = foreach ($_rsv in $reserved_info) {
    $applicant = fn_Array_Filter $_source_list $target_prop $_rsv.$target_prop;
    foreach ($_item in @('登録予約日', '登録予約時間', '管理会社')) {
      $applicant.$_item = $_rsv.$_item
    }
    $applicant
  }
  return $applicants
...
# CSVファイルを出力
fn_Export_CSV $applicants $CSV_destination
# JSONファイルを出力
fn_Export_JSON $applicants $JSON_destination
