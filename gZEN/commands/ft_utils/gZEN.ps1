...
function fn_Shape_Values {
  ...
  foreach ($_key in $_hash.keys) {
    ...
    Add-Member -InputObject $obj -NotePropertyMembers @{
      $_addition.company_name = $app_coms.($obj.'所属企業番号')
    }
    $new_hash[$_key] = $obj
  }
  return $new_hash
...
[PSCustomObject]$script:config = [FT_IO]::Read_JSON_Object($config_path)
$gZEN_config = $config.$command_name
$addition = $config.common_field
...
$selection_arr = [FT_IO]::Read_ToArray($gZEN_config.sorted_header, $txt_encoding) 

remove-variable -Name config_path, header, values, values_filenames, pan_folder

# TODO: リファクタリングすること
# 必要な項目の情報を抽出する
$shaped_hash = fn_Shape_Values $hash $gZEN_config $addition


# 出力プロパティを任意の順序で取得する。
$final_hash = [FT_Dict]::Selective($shaped_hash, $selection_arr)
remove-variable shaped_hash


$export_path = (${HOME} + $gZEN_config.export_JSON_path)
[FT_IO]::Write_JSON_Object($export_path, [PSCustomObject]$final_hash)
