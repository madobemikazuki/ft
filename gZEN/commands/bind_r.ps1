...
<#
機能 : gZEN_exported.json と 登録_予約日リスト_UTF8-bom.json の情報をバインドする。
...
[PSCustomObject[]]$script:gZEN_obj_list = fn_Read $gZEN_Path
#$gZEN_obj_list
...
$script:gZEN_key = $config.search_key
$script:addition_keys = $config.new_keys


$script:extracted_applicants = foreach ($_reserved in $reserved_obj_list) {
  if ($null -eq $_reserved.$gZEN_key) { continue }
  [PSCustomObject[]]$filtered_list = fn_Array_Filter $gZEN_obj_list $gZEN_key $_reserved.$gZEN_key
  $addition_reserved_obj = $_reserved | Select-Object -Property $addition_keys
  $extracted = foreach ($_obj in $filtered_list) {
    fn_Append_KV $_obj $addition_keys $addition_reserved_obj
  }
  $extracted  
}
...
foreach ($_ in   $extracted_applicants ) {
  $app_com_names = fn_Application_Company_Names $_
  #$_ | Add-Member -NotePropertyMembers @{$config.selection_key = $app_com_names } -Force
  Add-Member -InputObject $_ -NotePropertyName $config.selection_key -NotePropertyValue $app_com_names -Force
}

$selection = $extracted_applicants | Select-Object -Property $config.r_selection
