...
class FT_IO {
  ...
  static [void] Write_JSON([String]$_path, [PSCustomObject[]]$_Object_List) {
    if (Test-Path $_path) {
  ...
  static [String]Find_Latest_File([String]$_Folder, [String]$_TargetName) {
    ...
    if (($null -eq $file_list) -or !(Test-Path $file_list)) { throw "$_TargetName に該当するファイルが存在しません。" }
