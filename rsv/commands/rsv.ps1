...
#$select_rows_range
    ...
    $columns = @($_task.starting_column..$_task.end_column)
    #フィールド列の値を配列に格納
    ...
    [PScustomObject[]]$list = foreach ($_row in $select_rows_range) {
      ...
      foreach ($_column in $columns) {
        $_index = $columns.IndexOf($_column)
        ...
        $_value = switch -Regex ($_key) {
          ...
          Default { $Sheet.Cells.Item($_row, $_column).Text }
        }
        #ここで詰まっている。add-member 
        $obj | Add-Member -NotePropertyName $_key -NotePropertyValue $_value -Force
      }
      # 予約日が空の $obj は返さない。
