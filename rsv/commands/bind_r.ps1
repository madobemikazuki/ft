...
$gZEN_key = "中央登録番号"
$addition_keys = @("登録予約日", "登録予約時間", "管理会社")
#[PSCustomObject[]]$binded_list = fn_Insert_Reserved $gZEN_obj_list $reserved_obj_list $addition
#$binded_list

# いったんべた書き。
[PSCustomObject[]]$binded_list = foreach ($_applicant in $gZEN_obj_list){
  $new_info = $_applicant
  $reserve_info = $reserved_obj_list | Where-Object { $_.$gZEN_key -eq $new_info.$gZEN_key }
  foreach ($_key in $addition_keys){
    Add-Member -InputObject $new_info -NotePropertyName $_key -NotePropertyValue $reserve_info.$_key -Force
  }
  $new_info
}
...
foreach ($_ in $binded_list) {
  $new_obj = $_
  $app_com_names = fn_Application_Company_Names $new_obj
  Add-Member -InputObject $new_obj -NotePropertyName $config.selection_key -NotePropertyValue $app_com_names -Force
}
