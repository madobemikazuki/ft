...
<#
FT_Array]::Flat_KV($arr, $K, $V)でこんな形になる
{
  "app_coms":{
    "00000": "aaaaa",
  },
...
$export_path = (${HOME} + $config.export_path)
if (Test-Path $export_path) {
  ...
  foreach ($_class in $coms_class) {
    $every_true[$_class] = [FT_Object]::Compare_Count_Over($existing_Coms.$_class, $completion_obj.$_class)
  }
  # $every に $True が含まれていないなら何もせず終了する。
  # $every が全て$True であるなら反対の結果となる。
  # $True の反対であるか？=> True なのでこのif文が実行される
  # PowerShell に判別しやすいEvery メソッドは存在しない？
  if (!($False -in $every_true.Values)) {
    $private:marged_Coms = [FT_Object]::Marge($existing_Coms, $completion_obj)
    Write-Host "増えよ 🌳🌳🌳"
    [FT_IO]::Write_JSON_Object($export_path, $marged_Coms)
    exit 0
  }
  Write-Host "本日は晴天なり 🌞"
  exit 0
