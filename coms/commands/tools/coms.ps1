...
function private:fn_Extract_Set {
  ...
  $private:coms = . .\utils\ol.ps1 map $psc_list $keys
  $private:coms_set = . .\utils\ol.ps1 unique $coms $keys[0]
  return $coms_set
}
...
# 登録にも解除にも必要な情報
[PSCustomObject[]]$private:source = . .\ft_core\io\read_registed_people_fromT.ps1
...
. .\ft_core\io\write_json.ps1 "${HOME}\Downloads\TEMP\Applicate_Coms.json" $app_coms_kv
. .\ft_core\io\write_json_array.ps1 "${HOME}\Downloads\TEMP\Employ_Coms.json" $emp_coms
