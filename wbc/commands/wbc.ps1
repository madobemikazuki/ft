...
function fn_Unit_Processing {
  ...
  # $export_path に出力
  . .\ft_cores\Poe\Poe-Transcription.ps1 $posting_object $_config_object.poe_config $export_path
}
...
function fn_WBC_Format {
  ...
  # 該当しない人がいる場合は処理を中止する。
  if ([FT_Dict]::every($reserved_dict, $_central_num_list)) {
    ...
    Write-Host "該当者がいないため全ての処理を停止しました。" -ForegroundColor Red
    exit 404
  }
}

# -----------------------------------------------------------------
Write-Host $_central_nums.length
...
[PSCustomObject[]]$applicants = fn_WBC_Format $_central_nums $config_object.$Task $printing_field
