...
function script:fn_shape_values {
  ...
  # TODO: gZENに書くべきですね。
  # $config.paths.gZEN_exported に'T***'で参照してもエラーなら、
  # メモ帳を起動する。通知する。書き足せと。
  # Start-Process notepad (${home} + $config.paths.application_coms)
  # exit 0
  ...
  $new_obj = $new_obj | Select-Object *, @{
    ...
    Expression = {
      ...
      $private:shorten_2 = if ([String]::IsNullOrEmpty($_.'雇用企業名称（漢字）')){
        return fn_Shorten_Com_Type_Name $_.'所属企業名'
      } else{
        return fn_Shorten_Com_Type_Name $_.'雇用企業名称（漢字）'
      }
