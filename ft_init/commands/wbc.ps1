...
function fn_Registration_Format {
  ...
  [PSCustomObject[]]$private:applicant_list = foreach ($_ in $extracted_targets_info) {
    [PSCustomObject] @{
      ...
      $script:wbc_application_field[0] = fn_Excel_Hell_Format ([DateTime]$_.'登録予約日')
      $script:wbc_application_field[1] = $_.'個人番号'
      $script:wbc_application_field[2] = fn_Shorten_Com_Type_Name $_.'登録_申請会社'
...
function fn_Cancellation_Format {
  ...
  [PSCustomObject[]]$private:applicants = foreach ($_ in $extracted_targets_info) {
    ...
    [PSCustomObject] @{
      # TODO:wbc_application_field を外部スコープに依存しているのがきもいなぁ。
      $script:wbc_application_field[0] = fn_Excel_Hell_Format ([DateTime]$_."解除予約日")
      $script:wbc_application_field[1] = $_."作業者証番号"
      
      # ◆未実装  所属企業番号から '申請会社名' を取得すること 
      $script:wbc_application_field[2] = fn_Shorten_Com_Type_Name $_.'解除WBC_申請会社'
      $script:wbc_application_field[3] = $_.'漢字氏名'
    }
  }
  return $applicants
