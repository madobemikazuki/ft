...
function private:fn_Extract_Registered_Targets {
  ...
  [PSCustomObject[]]$list = foreach ($_person in $_registered_persons) {
    $shorten_1 = fn_Shorten_Com_Type_Name $_person."電力申請会社名称"
    $shorten_2 = fn_Shorten_Com_Type_Name $_person."雇用名称"
    $full_name = fn_Combined_Name $_person."氏名（姓）" $_person."氏名（名）"
  
    [PSCustomObject]@{
      "登録_申請会社" = fn_Application_Company_Names $shorten_1 $shorten_2
